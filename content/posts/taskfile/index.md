...
No i denerwowała mnie ta dziwna składnia z tabami, z którą był problem na różnych edytorach 🤷
...
przeczytać [tutaj](https://taskfile.dev/usage/#using-programmatic-checks-to-indicate-a-task-is-up-to-date).
...
Więcej [tutaj](https://taskfile.dev/usage/#limiting-when-tasks-run).

#### `start` i `build`

Tutaj żadnej filozofi nie ma. Proste zadania z zależnością do `init`. Oczywiście przy wywołaniu obu tych zadań będziemy
...
w [dokumentacji](https://taskfile.dev). Warto jednak wymienić kilka ciekawszych:

* ☑️ [Wykorzystanie plików .env](https://taskfile.dev/usage/#env-files)
* ☑️ [Korzystanie z zewnętrznych plików z zadaniami](https://taskfile.dev/usage/#including-other-taskfiles)
* ☑️ [Aliasy poleceń](https://taskfile.dev/usage/#task-aliases)
* ☑️ [Parametry w nazwie zadania](https://taskfile.dev/usage/#wildcard-arguments)
* ☑️ [Czyszczenie po zadaniu](https://taskfile.dev/usage/#doing-task-cleanup-with-defer)
* ☑️ [Opisywanie tasków (dokumentacja)](https://taskfile.dev/usage/#help)
